...
SyncSrc::~SyncSrc()
{
    assert(mcpwm_del_sync_src(_handle));
    _handle = nullptr;
}
...
Fault::~Fault()
{
    assert(mcpwm_del_fault(_handle));
    _handle = nullptr;
}
...
{
    assert(mcpwm_del_comparator(_handle));
    _handle = nullptr;
}
...
{
    assert(mcpwm_del_generator(_handle));
    _handle = nullptr;
}
...
{
    assert(mcpwm_del_capture_timer(_handle));
    _handle = nullptr;
}
...
{
    assert(mcpwm_del_capture_channel(_handle));
    _handle = nullptr;
}
