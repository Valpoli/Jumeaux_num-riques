...
void IrProximitySensor::update(float, const sf::Transform & parentWorldTransform)
{
    ...
    //compute _distanceDetected and _line[1].position from _distanceMax, _map, worldTranform, position

    // Begin stub
    sf::Vector2f position = _line[0].position;
    sf::Vector2f direction = sf::Vector2f(cos(getRotation() * M_PI / 180), sin(getRotation() * M_PI / 180));
    sf::Vector2f position2 = position + direction * _distanceMax;

    _distanceDetected = 0;
    while (_distanceDetected < _distanceMax && _map.getPixel(position2.x, position2.y) < 255)
    {
        _distanceDetected += 1;
        position2 += direction;
    }

    _line[1].position = position + direction * _distanceDetected;
    // End stub
    ...
    {
        _distanceDetected = 0;
        while (_distanceDetected < _distanceMin && _map.getPixel(position2.x, position2.y) < 255)
        {
            _distanceDetected += 1;
            position2 += direction;
        }
